...
from tinygrad.ops import GroupOp, Ops, PatternMatcher, UOp, UPat, graph_rewrite
    ...
    for u in x.src:
        if u.op is Ops.BLOCK:
            if u not in seen_u:
                ...
                assert (
                    u.arg.ctx not in old_blocks
                ), "sibling should never have been created"
...
def make_basic_blocks_with_profile(uop, ctx):
    # Start timing the matcher to profile how long the rewrite pattern takes
    t0 = time.perf_counter()

    # Pattern matcher that defines how to lower UOps into basic blocks
    pattern = PatternMatcher(
        [
            # Pattern 1: wrap final SINK into a BLOCK, optionally prepending a NAME
            (
            ...
            # Pattern 2: extend existing BLOCKs with more ops (and possibly new context)
            (UPat(Ops.BLOCK, name="x"), append_to_block),
        ]
    )

    # Log when profiling is active for the basic_blocks matcher
    print(
        f"[prof] make_basic_blocks_with_profile: PatternMatcher returned for graph_rewrite"
    )
    return pattern
...
def block_merge(ctx, x: UOp):
    ...
    if x.op is Ops.BLOCKEND:
        ...
        if len([y for y in ctx[x.arg.end] if y not in in_this_block]) == 0:
            ...
            if len(parent_blocks) == 1:
                ...
                if (
                    x.op is Ops.BLOCKEND
                    and any(y.op is Ops.BARRIER for y in late_ops)
                    and late_ops[-1].op is Ops.ENDRANGE
                ):
    ...
    return UOp(
        ...
        tuple(new_srcs),
        BasicBlock(
            tuple(dedup(sorted(new_ctx, key=lambda x: x.tuplize))),
...
pm_block_merge = PatternMatcher(
    [
        ...
        (
            UPat(Ops.BLOCKFORK, name="f", src=(UPat(Ops.BLOCKFORK, name="f2"))),
            lambda f, f2: f.replace(src=f2.src, arg=f.arg * f2.arg),
        ),
    ]
)
...
def upsettingly_promote_blockend(be: UOp):
    new_srcs = tuple(
        b.replace(arg=BasicBlock(be.arg.ctx, b.arg.lst)) if b.op is Ops.BLOCK else b
        for b in be.src
    )
    return be.replace(src=new_srcs) if be.src != new_srcs else None


pm_force_upcast_block = PatternMatcher(
    [(UPat(Ops.BLOCKEND, name="be"), upsettingly_promote_blockend)]
)


def linearize_uop(sink: UOp, skip_check: bool = not __debug__) -> list[UOp]:
    # start timing for perf tracking
    t0 = time.perf_counter()

    # tracing where all these linearize_uop calls are coming from
    print("[trace] linearize_uop was called from:")
    traceback.print_stack(limit=4)

    # sanity check: input op must be a SINK to begin lowering
    assert sink.op is Ops.SINK, f"sink isn't sink, it's {sink.op}"

    # timing just the graph lowering and rewrite phase
    ...
    # --[ TIME THIS LOOP CAREFULLY ]--
    while_loop_t0 = time.perf_counter()

    # TODO: there's probably a clever way to remove this while loop
    while 1:
        # --- START profiling: measure time for first block rewrite phase only ---
        start_basic_blocks = time.perf_counter()

        # This shows how many UOps are in the graph at this point
        print(f"[debug] UOp graph size before basic_blocks: {len(sink.toposort)}")

        # count how many BLOCK ops — these are slow to process
        print(
            f"[debug] num BLOCKs: {sum(1 for u in sink.toposort if u.op is Ops.BLOCK)}"
        )

        # count IFs — control flow usually adds complexity
        print(f"[debug] num IFs: {sum(1 for u in sink.toposort if u.op is Ops.IF)}")

        # END RANGE pairs with IFs — these help end conditionals/loops
        print(
            f"[debug] num ENDRANGEs: {sum(1 for u in sink.toposort if u.op is Ops.ENDRANGE)}"
        )

        # original graph rewrite that lowers the graph into basic blocks
        sink = make_basic_blocks_with_profile(sink, ctx=(block_ctxs, children))

        # --- END profiling: log how long lowering into basic blocks took ---
        print(
            f"[prof] make_basic_blocks rewrite took {(time.perf_counter() - start_basic_blocks)*1000:.2f} ms"
        )
        ...
        for u, child_count in block_parent_count.items():
            if (
                u.op not in DONT_PLACE_IN_BLOCK
                and child_count > 1
                and u not in non_block_parents
            ):
                # TODO: this is copied from append_to_block
                new_block = UOp(
                    Ops.BLOCK, src=u.src, arg=BasicBlock(block_ctxs[u], (u,))
                )
                rng = block_ctxs[u]
                lrng = list(rng)
                for r in rng[::-1]:
                    ...
                    if (
                        all(r not in block_ctxs[c] for c in children[u])
                        and r.op is not Ops.BLOCKSTART
                    ):
                        lrng.remove(r)
                        new_block = UOp(
                            Ops.BLOCKEND,
                            ...
                            arg=BasicBlock(
                                ...
                                (
                                    UOp(
                                        Ops.ENDIF if r.op is Ops.IF else Ops.ENDRANGE,
                                        src=(r,),
                                    ),
                                ),
                                r,
        ...
        # --- start profiling: fork substitution timing (already-existing substitution line) ---
        start_fork_rewrite = time.perf_counter()

        # note: this substitution call came with the repo — just measuring it
        sink = sink.substitute(forks)

        # log how long fork substitution took
        print(
            f"[prof] fork substitution rewrite took {(time.perf_counter() - start_fork_rewrite)*1000:.2f} ms"
        )

    # time spent inside the rewrite loop — this can get expensive with deep graphs
    print(
        f"[prof] while rewrite loop took {(time.perf_counter() - while_loop_t0)*1000:.2f} ms"
    )
    ...
    # done with lowering — time everything from beginning and from rewrite phase
    t_final = time.perf_counter()
    print(f"[prof] linearize_uop total took {(t_final - t0)*1000:.2f} ms")
    print(f"[prof] graph rewrite + lowering took {(t_final - rewrite_t0)*1000:.2f} ms")
