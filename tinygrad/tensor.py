...
def _metaop(
    op, shape: tuple[sint, ...], dtype: DType, device: str | tuple[str, ...], arg=None
) -> UOp:
...
def _frompy(x: list | tuple | bytes, dtype: DType) -> UOp:
    if isinstance(x, bytes):
        ...
        ret = UOp.new_buffer("PYTHON", prod(shape := get_shape(x)), dtype).reshape(
            shape
        )
...
class Tensor(SimpleMathTrait):
    ...
    def __init__(
        ...
        if isinstance(data, UOp):
            ...
            data = _metaop(
                Ops.CONST, (0,), dtype or dtypes.default_float, device, arg=0
            )
            ...
            data = UOp.new_buffer(
                f"DISK:{data.resolve()}", data.stat().st_size // dtype.itemsize, dtype
            )
    ...
    def schedule_with_vars(
        ...
        """
        # DEBUG: Print LazyData from all input Tensors to verify nothing is None
        print(">>> LazyData check:", [x.lazydata for x in (self,) + lst])

        # DEBUG: Confirm whether UOp.sink(...) actually returns a valid sink
        print(">>> big_sink:", big_sink)  # <-- add this
    ...
    def replace(self, x: Tensor, allow_shape_mismatch=False) -> Tensor:
        ...
        assert (
            self.shape == x.shape or allow_shape_mismatch
    ...
    def assign(self, x) -> Tensor:
        ...
        if isinstance(self.device, str) and self.device.startswith("DISK"):
            ...
            self.contiguous().realize().lazydata.base.buffer.ensure_allocated().copyin(
                x._data()
            )
            return self
        if x.__class__ is not Tensor:
    ...
    def _buffer(self) -> Buffer:
        return (
            self.cast(self.dtype.base)
            .contiguous()
            .to("CPU")
            .realize()
            .lazydata.base.buffer
        )
        ...
        **kwargs,
        ...
        if isinstance(device, tuple):
            return Tensor(
                UOp.multi(
                    *[
                        UOp.new_buffer(Device.canonicalize(d), size, dtype).reshape(
                            shape
                        )
                        for d in device
                    ],
        ...
        return Tensor(
            UOp.new_buffer(Device.canonicalize(device), size, dtype),
            device,
            dtype,
            **kwargs,
        ).reshape(shape)
    ...
    def roll(
        self, shifts: int | tuple[int, ...], dims: int | tuple[int, ...]
    ) -> Tensor:
        """
    ...
    def masked_select(self, mask):
        ...
        if not dtypes.is_bool(mask.dtype):
            raise RuntimeError(
                f"masked_select expects bool mask tensor, got {mask.dtype}"
            )
    ...
    def sum(
        ...
        return (
            ret.cast(self.dtype)
            if dtype is None and self.dtype in (dtypes.float16, dtypes.bfloat16)
            else ret
        )
    ...
    def max_unpool2d(
        ...
        ret = (
            indices.reshape(bs, c, 1, -1)._one_hot_along_dim(prod(output_size), 2)
            * self.reshape(bs, c, 1, -1)
        ).sum(3)
    ...
    def conv2d(
        ...
        ret = _apply_winograd_matrix(
            winograd_At,
            (gfactors * dfactors).sum(axis=-1 - len(HW), dtype=dtype),
            len(HW),
        )
    ...
    def sort(self, dim: int = -1, descending: bool = False) -> tuple[Tensor, Tensor]:
        ...
        for stage in range(1, n_stages + 1):
            if stage != n_stages:
                ...
                x = (
                    blue_box.cat(green_box.flip(flip_dims), dim=crossover_dim)
                ).contiguous()
        ...
        x = x.flatten(dim, dim + n_stages - 1).shrink(
            tuple((0, orig_len) if i == dim else None for i in range(x.ndim))
        )
        # compute indices for sorted values
        idx = Tensor.arange(orig_len, requires_grad=False, device=self.device).reshape(
    ...
    def __lshift__(self, x: int) -> Tensor:
        return self.lshift(x)

    def __rshift__(self, x: int) -> Tensor:
    ...
    def __ixor__(self, x) -> Tensor:
        return self.assign(self.bitwise_xor(x))

    def __ilshift__(self, x: int) -> Tensor:
        return self.assign(self.lshift(x))

    def __irshift__(self, x: int) -> Tensor:
    ...
    def scaled_dot_product_attention(
        ...
        qk = self.matmul(
            key.transpose(-2, -1),
            dtype=least_upper_dtype(self.dtype, key.dtype, dtypes.float32),
